...
def eval(epoch, video_encoder, seeg_encoder, eval_loader, writer, device, split):
    ...
    with torch.no_grad():
        for video, seeg, seeg_padding_mask in tqdm(eval_loader):
        ...
        if split == 'val':
            writer.add_scalar(f'Val/Acc@1 for Each Epoch', acc1, epoch)
            writer.add_scalar(f'Val/Acc@2 for Each Epoch', acc2, epoch)
